...
public class LockscreenStyle extends SettingsPreferenceFragment
    ...
    private void updateLockSummary() {
        ...
        } else if (value.contains("carbon_lock")) {
            resId = R.string.lockscreen_lock_icon_carbon;
    ...
    private void resizeCarbonLock() {
        Bitmap carbonLock = BitmapFactory.decodeResource(getResources(), R.drawable.carbon_lock);
        if (carbonLock != null) {
            String path = null;
            int px = requestImageSize();
            carbonLock = Bitmap.createScaledBitmap(carbonLock, px, px, true);
            try {
                mLockImage.createNewFile();
                mLockImage.setWritable(true, false);
                File image = new File(getActivity().getFilesDir() + File.separator
                            + "carbon_lock" + System.currentTimeMillis() + ".png");
                path = image.getAbsolutePath();
                mLockImage.renameTo(image);
                FileOutputStream outPut = new FileOutputStream(image);
                carbonLock.compress(Bitmap.CompressFormat.PNG, 100, outPut);
                image.setReadable(true, false);
                outPut.flush();
                outPut.close();
            } catch (Exception e) {
